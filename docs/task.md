...
- [ ] **Enable Next 16 cache components**: `next.config.ts` does not set `cacheComponents: true`, so the app is not using Next 16's cache-component/static-shell navigation model yet. (Deferred until the remaining client pages finish their RSC pass — it flips PPR on app-wide and is safest after route cleanup.)
...
- [ ] **Convert remaining client-fetch dashboard pages to RSC where practical**: `dashboard`, `campaign detail`, and `diagnostics` are now Server Components (see below). `overview` and `settings` remain client-fetch for now: overview's Meta pipeline + refresh/race-guard logic, and settings' form-heavy mutations, make them lower-value conversions.
...
- [ ] **Server Components for DB dashboard pages**: Move DB-backed dashboard screens to RSC/direct Prisma where possible (`dashboard`, `campaigns`, `logs`, `settings`, `diagnostics`, campaign detail). Keep only interactive controls as client islands.
...
- [ ] **Next 16 `cacheComponents` after route cleanup**: Enable `cacheComponents: true` once the main dashboard routes have clear Suspense boundaries and dynamic data is intentionally cached or streamed.
