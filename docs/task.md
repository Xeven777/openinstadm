...
- [x] **Multi-row CSV import leaks stale state between rows**: fixed 2026-08-26. `prefillFromRow` resets every option not represented in the CSV before loading the next row.
- [ ] **Can't toggle a broken campaign in edit mode**: "Stop"/"Go Live" ([`components/campaign-builder.tsx`](/components/campaign-builder.tsx#L581)) run the full validation in `handleSubmit` ([`L366`](/components/campaign-builder.tsx#L366)), so a campaign with e.g. an empty DM message can never be paused/activated — the validation error returns before `isActive` is applied. Toggle should be a lightweight `isActive`-only PATCH.
- [ ] **`{link}` token only inserted on URL blur — preview lies**: `ensureLinkToken` ([`components/campaign-builder.tsx`](/components/campaign-builder.tsx#L362)) fires only on `onBlur` of the URL input ([`L867`](/components/campaign-builder.tsx#L867)). Saving without blurring leaves `dmMessage` with no `{link}`; the preview then shows no link card (`showCard = hasLink && hasToken`, [`campaign-preview.tsx`](/components/campaign-preview.tsx#L410)) while the worker still sends the link button (`buildLinkButtons` keys off `trackedLinks`, [`lib/queue/dm-worker.ts`](/lib/queue/dm-worker.ts#L84)). Preview ≠ reality.
- [x] **`postCaption` not cleared on account switch**: fixed 2026-08-26. Changing accounts clears the selected post caption with the other post state.
- [ ] **Edit mode preview has a blank post image**: the prefill effect ([`components/campaign-builder.tsx`](/components/campaign-builder.tsx#L224)) never populates `postThumb` (the list payload has no thumbnail), so the Post preview is an empty black box.
- [ ] **No client-side keyword limits**: server caps 10 keywords / 50 chars each ([`app/api/automations/route.ts`](/app/api/automations/route.ts#L36)); the input ([`components/campaign-builder.tsx`](/components/campaign-builder.tsx#L692)) allows unlimited, so an 11th keyword fails with a generic "Invalid input" after a full round-trip.
- [x] **Stale post on trigger-scope switch**: fixed 2026-08-26. Changing to any/next clears all selected-post state.
- [ ] **Minor**: `publicReplyMessages` keyed by index `i` ([`components/campaign-builder.tsx`](/components/campaign-builder.tsx#L736)) → inputs can lose focus on delete; the custom `Radio`/`Switch` rows lack `role="radio"`/`aria-checked`/proper labels.
...
- [x] **Repeated heavy `/api/automations` fetches**: the used-posts effect ([`components/campaign-builder.tsx`](/components/campaign-builder.tsx#L279)) fetches the full campaign list with `cache: "no-store"` on mount *and on every account switch*; `getCampaignList` is the heaviest query in the app (automations + 3 `groupBy` analytics aggregations, [`lib/server/automations.ts`](/lib/server/automations.ts#L207)). In edit mode it doubles with the prefill fetch ([`L224`](/components/campaign-builder.tsx#L224)) — 2 identical heavy calls. No react-query cache, no dedupe, no debounce, and only `{id, name, postId, instagramAccountId}` is needed. → **Fixed 2026-08-17**: added lightweight `GET /api/automations?fields=used-posts` backed by `getCampaignUsedPosts` ([`lib/server/automations.ts`](/lib/server/automations.ts)) — 4-column `select` (no includes, no groupBy), `use cache` + `campaigns:{workspaceId}` tag with `?instagramAccountId` filter, consumed via `queryKeys.usedPosts` + `fetchUsedPosts` ([`lib/query/keys.ts`](/lib/query/keys.ts), [`lib/query/api.ts`](/lib/query/api.ts)) with `staleTime 60s` deduped by TanStack Query keyed by account.
- [x] **Use `getCampaignDetail` for edit prefill**: [`lib/server/automations.ts`](/lib/server/automations.ts#L276) already exposes it — the builder fetches the whole list to find one campaign ([`L226`](/components/campaign-builder.tsx#L226)). → **Fixed 2026-08-17**: `GET /api/automations?id=` now serves `getCampaignDetail` ([`app/api/automations/route.ts`](/app/api/automations/route.ts)); builder prefills via `queryKeys.campaignDetail` + `fetchCampaignDetail` with `staleTime 60s` (single-row query, no analytics fan-out for list).
- [x] **PostPicker eagerly renders up to 300 thumbnails**: `all: true` paginates the full library ([`lib/server/instagram-media.ts`](/lib/server/instagram-media.ts#L106)) and every `<img>` renders without `loading="lazy"` ([`components/post-picker.tsx`](/components/post-picker.tsx#L182)), in a `max-h-64` scroll. → **Fixed 2026-08-17**: the picker now fetches a **thumbnail-only** field set (new `MEDIA_THUMB_FIELDS` in [`lib/meta/client.ts`](/lib/meta/client.ts), passed through `getAllUserMedia` — `media_url` never reaches the client), and the grid **paginates client-side at 10/page** with a "Show more" button over the IndexedDB-persisted TanStack Query cache; images are `loading="lazy"` + `decoding="async"`.
- [x] **Video hover starts streaming immediately**: `onMouseEnter` mounts an autoplay `<video>` ([`components/post-picker.tsx`](/components/post-picker.tsx#L189), [`L228`](/components/post-picker.tsx#L228)) with no debounce; fast hovering across a grid of reels fires several concurrent reel downloads. → **Fixed 2026-08-17**: hover video preview removed entirely — the picker renders cover frames only (no `media_url` in the payload, so video streaming is impossible).
- [x] **Whole phone mockup re-renders per keystroke**: fixed 2026-08-26. `CampaignPreview` is memoized, so fields that do not affect the preview no longer redraw the phone or its SVG icon tree.
- [ ] **Align preview with worker**: render the link button card whenever `hasLink` regardless of token, and/or auto-insert `{link}` on submit + client-validate (see bug above).
...
Big lists should render only visible items plus intentional overscan, measure dynamic content, and restore scroll anchors on prepends/streaming. `@tanstack/react-virtual` is installed. The posts list is contained in a self-scrolling `max-h-[800px]` box, so the virtualizer scrolls with that box directly — no page-offset `scrollMargin` measurement needed.

- [x] **Virtualize overview posts (table + grid)**: [`components/overview-posts-view.tsx`](/components/overview-posts-view.tsx) contains both views in a `max-h-[800px]` scroll box and virtualizes against it with `useVirtualizer`. Table = fixed-height estimate rows under a sticky header (the header height becomes the virtualizer `scrollMargin`, since virtual rows aren't the box's first child); grid = responsive rows (2/3/4 cols mirrored from the Tailwind breakpoints) measured with `measureElement`. Up to 500 posts (`OVERVIEW_MAX_POSTS`) are no longer mounted at once. Scroll position is snapshotted per post-set signature (length + newest post id) in-memory and sessionStorage, and restored on account/range switch and table↔grid toggle. Semantic table markup kept via ARIA `role` attributes.
