...
The four jobs under `/api/cron` are scheduled by the `crons` block in
...
- **`snapshot-followers`** keeps the follower history, which Instagram only
  retains for ~30 days.
- **`snapshot-cleanup`** removes expired API snapshots. It is not critical to
  delivery, but without it stale rows can accumulate.
...
The script calls `attach-next-reel` every five minutes and the other three once
a day. Five minutes is deliberate — on Vercel this runs daily, which means a
